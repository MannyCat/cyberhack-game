-- ============================================================================
-- Migration 003: Missing Tables for CyberHack Game
...
-- 3a. campaigns — server-managed, read-only for all
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "campaigns_no_insert" ON public.campaigns
    FOR INSERT WITH CHECK (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "campaigns_no_update" ON public.campaigns
    FOR UPDATE USING (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "campaigns_no_delete" ON public.campaigns
    FOR DELETE USING (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3b. campaign_progress — own data
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "campaign_progress_insert_own" ON public.campaign_progress
    FOR INSERT WITH CHECK (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "campaign_progress_update_own" ON public.campaign_progress
    FOR UPDATE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "campaign_progress_delete_own" ON public.campaign_progress
    FOR DELETE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3c. player_programs — own data
DO $$ BEGIN
  CREATE POLICY "player_programs_select_own" ON public.player_programs
    FOR SELECT USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "player_programs_insert_own" ON public.player_programs
    FOR INSERT WITH CHECK (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "player_programs_update_own" ON public.player_programs
    FOR UPDATE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "player_programs_delete_own" ON public.player_programs
    FOR DELETE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3d. training_queue — own data
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "training_queue_insert_own" ON public.training_queue
    FOR INSERT WITH CHECK (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "training_queue_update_own" ON public.training_queue
    FOR UPDATE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "training_queue_delete_own" ON public.training_queue
    FOR DELETE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3e. weekly_events — server-managed, read-only for all
DO $$ BEGIN
  CREATE POLICY "weekly_events_select_all" ON public.weekly_events
    FOR SELECT USING (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "weekly_events_no_insert" ON public.weekly_events
    FOR INSERT WITH CHECK (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "weekly_events_no_update" ON public.weekly_events
    FOR UPDATE USING (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "weekly_events_no_delete" ON public.weekly_events
    FOR DELETE USING (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3f. event_participation — own data
DO $$ BEGIN
  CREATE POLICY "event_participation_select_own" ON public.event_participation
    FOR SELECT USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "event_participation_insert_own" ON public.event_participation
    FOR INSERT WITH CHECK (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "event_participation_update_own" ON public.event_participation
    FOR UPDATE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "event_participation_delete_own" ON public.event_participation
    FOR DELETE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3g. daily_rewards — own data
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "daily_rewards_delete_own" ON public.daily_rewards
    FOR DELETE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 3h. achievements — server-managed, read-only for all
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "achievements_no_insert" ON public.achievements
    FOR INSERT WITH CHECK (false);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
-- 3i. player_achievements — own data
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "player_achievements_update_own" ON public.player_achievements
    FOR UPDATE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "player_achievements_delete_own" ON public.player_achievements
    FOR DELETE USING (auth.uid() = player_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
VALUES
  ('c3d4e5f6-0001-4000-8000-000000000001', 'first_node',     'Первый узел',      'Разверните первый сетевой узел',       '📡', 'network', 200,   40,   1),
  ('c3d4e5f6-0002-4000-8000-000000000002', 'network_builder', 'Строитель сети',    'Разверните 10 узлов',                   '🌐', 'network', 1000,  200,  2),
  ('c3d4e5f6-0003-4000-8000-000000000003', 'first_attack',    'Первый взлом',      'Проведите первую атаку',               '⚔️', 'combat',  300,   50,   3),
  ('c3d4e5f6-0004-4000-8000-000000000004', 'cyber_warrior',   'Кибервоин',         'Проведите 100 атак',                   '🛡️', 'combat',  5000,  1000, 4),
  ('c3d4e5f6-0005-4000-8000-000000000005', 'wealthy_hacker',  'Богатый хакер',     'Накопите 100,000 кредитов',            '💰', 'economy', 3000,  500,  5),
  ('c3d4e5f6-0006-4000-8000-000000000006', 'market_shark',    'Акула рынка',       'Купите 20 товаров',                    '🛒', 'economy', 2000,  400,  6),
  ('c3d4e5f6-0007-4000-8000-000000000007', 'clan_founder',    'Основатель',        'Создайте клан',                        '👑', 'social',  5000,  500,  7),
  ('c3d4e5f6-0008-4000-8000-000000000008', 'clan_leader',     'Лидер клана',       'Достигните 10 уровня клана',           '🏆', 'social',  8000,  800,  8),
  ('c3d4e5f6-0009-4000-8000-000000000009', 'high_level',      'Ветеран',           'Достигните 25 уровня',                 '⭐', 'special', 10000, 2000, 9),
  ('c3d4e5f6-0010-4000-8000-000000000010', 'legend',          'Легенда',           'Достигните 50 уровня',                 '🏆', 'special', 50000, 5000, 10)
ON CONFLICT DO NOTHING;
...
-- 5b. claim_daily_reward(p_player_id uuid) — daily streak reward system
CREATE OR REPLACE FUNCTION public.claim_daily_reward(p_player_id uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=''
AS $$
DECLARE
  v_reward daily_rewards%ROWTYPE;
  v_tier_credits int;
  v_tier_xp int;
  v_tier_day int;
...
BEGIN
  -- Get current state
  SELECT * INTO v_reward FROM daily_rewards WHERE player_id = p_player_id;
  ...
  IF v_reward IS NULL THEN
    ...
    INSERT INTO daily_rewards (player_id, streak_day, current_streak, best_streak, total_claimed)
    VALUES (p_player_id, v_tier_day, 1, 1, 1) RETURNING * INTO v_reward;
  ELSE
    -- Check if streak is broken (last claim was not yesterday)
  ...
  -- Award resources
  UPDATE profiles SET credits = credits + v_tier_credits, experience = experience + v_tier_xp WHERE id = p_player_id;
...
-- 5c. join_event(p_player_id uuid, p_event_id uuid) — join an event
CREATE OR REPLACE FUNCTION public.join_event(p_player_id uuid, p_event_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path=''
AS $$
BEGIN
  INSERT INTO event_participation (player_id, event_id)
